...
int main()
{
	...
	// enable depth testing so z-position of textures doesn't get screwed
	glEnable(GL_DEPTH_TEST);

	// build and compile our shaders
	Shader ourShader("spincubeshader.vs", "spincubeshader.fs");

	// set up vertex data (and buffer(s)) and configure vertex attributes
	// in the beginning, there were the CUBES
	float vertices[] = {
	...
	// set world space positions of our cubes
	...
	stbi_set_flip_vertically_on_load(true); // make stb_image.h flip textures on the y-axis.
	unsigned char *data = stbi_load("spincubecontainertex.jpg", &width, &height, &nrChannels, 0);
	if (data)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ourShader.setMat4("projection", projection); // this is the brutish way, setting the projection matrix each frame.
		...
		{			
			...
			// divide boxes into 2 groups
			if (i % 2 == 0)
				// rotate each box with time			
				model = glm::rotate(model, (float)glfwGetTime(), glm::vec3(0.8f, 0.5f, 0.0f));
			else
				// rotate each box with time			
				model = glm::rotate(model, (float)glfwGetTime(), glm::vec3(0.2f, 1.0f, 0.0f));
